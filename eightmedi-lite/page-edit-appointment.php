...
$visit = $wpdb->get_results("SELECT id, description FROM wp_appointments WHERE id =". $vid);
$sched = $wpdb->get_results("SELECT fromDate, hours FROM wp_working_schedules WHERE doctor_id = ". $doctorID." ORDER BY fromDate ASC");
?>

<form action="update-appointment" method="post" name="form3" id="specanddoc">

<div id="appMake">
<h2> Please select a new date of a visit </h2><br><br>
    ...
    jQuery('#txtDate').datepicker({
        onSelect: function(dateText, inst) {
            ...
            if (selectList) {
                ...
                textArea.value = visit[0].description;
                ...
                butt.setAttribute("value", "Edit appointment");
                ...
                myDiv.appendChild(butt);
                butt = document.createElement("input");
                butt.setAttribute("name", "vid");
                butt.setAttribute("value", visit[0].id);
                butt.setAttribute("type", "hidden");
                myDiv.appendChild(butt);
