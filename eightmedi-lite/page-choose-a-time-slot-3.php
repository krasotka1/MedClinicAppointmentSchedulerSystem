...
'role' => 'subscriber',
 ...
 foreach ($users as $user) {
   ...
   $patients[$aux]['name'] = $user->display_name;
   $aux = $aux + 1;
   }
?>

<form action="choose-a-time-slot-3/appointment-added-3" method="post" name="form3" id="specanddoc">
...
jQuery(document).ready(function($) {
    ...
    for (var i = 0; i < patients.length; i++) {
        patNames[i] = patients[i].name +' ('+patients[i].dni+')';
    }
    var nwd = [];
function getSchedule(dateS){
    ...
    jQuery('#txtDate').datepicker({
        onSelect: function(dateText, inst) {
            ...
            if (selectList) {
                ...
                myDiv.appendChild(butt);
                butt = document.createElement("input");
                butt.setAttribute("name", "patID");
                butt.setAttribute("id", "patID");
    ...
    $( "#persName" ).autocomplete({
      ...
      select: function(e, ui){
          ...
          var pat = document.getElementById("patID");
          pat.value = patients[ind].id;
      }
      });
      });
            }
    ...
    if(document.getElementById("patID").value == '') {
        alert("A patient was not selected. Please, select first a patient before proceeding");
        return false;
    }
    return true;
    });    
