...
jQuery( document ).ready(function( $ ) {
    var docsInfo = <?php echo json_encode($docNames); ?>;
    var docNames = [];
    for (var i = 0; i < docsInfo.length; i++) {
        docNames[i] = docsInfo[i].name;
    }
...
$('#myform input').on('change', function() {
   if(($('input[name=choose]:checked', '#myform').val()) == 'clin') {
   document.getElementById('docName').style.display = 'none'; 
   document.getElementById('docName').required = false; 
   document.getElementById('docName').value = '';
   }
   else if(($('input[name=choose]:checked', '#myform').val()) == 'doc') {
   document.getElementById('docName').style.display = 'inline';
   document.getElementById('docName').required = true; 
   }
});
...
<body>
<form id="myform" action="holiday-added" method="POST">
<p><h1> Add a holiday form</h1>
<label for="doc"><h2>Holiday for:</h2></label> 
<p><input type="radio" name="choose" value="doc">Doctor<br>
    <input type="radio" name="choose" value="clin">Whole clinic<br>
</p>
<input type="text" id="docName" placeholder="Doctor's name" style="display:none"> 
<label for="datepicker"><h2>Date:</h2></label> 
<label for="from">From:</label>
<input type="text" id="from" name="from">
<label for="time">Time:</label>
<input type="text" id="time" name="time"placeholder="HH:MM format *Leave empty if for the whole day">
