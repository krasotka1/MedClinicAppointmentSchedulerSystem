...
<form action="choose-a-time-slot-2" method="post" name="form2" id="specanddoc">
<div id="appMake">
...
$rowcount = $wpdb->get_results("SELECT id FROM wp_appointments WHERE ip_address = '$ip' AND patient_id IS NULL");
if(count($rowcount) >= 5){ //More than 5 appointments. Don't let them make appointments
echo "<h3> You can't make more than 5 appointments as a guest user. Please register or attend some appointments before making new ones </h3>";
}
else{
